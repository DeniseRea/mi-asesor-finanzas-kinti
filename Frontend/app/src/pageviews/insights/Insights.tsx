...
export function Insights() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.15fr_1fr_.6fr]">
      ...
      <AppCard className="p-5"><h2 className="font-bold">Gastos por categoría</h2><div className="relative h-72"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} dataKey="value" cy="42%" innerRadius="45%" outerRadius="70%"></Pie><Tooltip formatter={(value) => formatMoney(Number(value))}/><Legend/></PieChart></ResponsiveContainer><div className="pointer-events-none absolute left-1/2 top-[42%] -translate-x-1/2 -translate-y-1/2 text-center"><strong className="block text-lg text-slate-950">{formatMoney(summary.expenses)}</strong><span className="text-[11px] text-slate-500">Total gastado</span></div></div></AppCard>
      <AppCard className="bg-linear-to-br from-emerald-50 to-lime-50 p-5"><span className="text-xs font-bold uppercase tracking-wide text-emerald-700">Insight destacado</span><h2 className="mt-5 text-lg font-bold text-emerald-950">Gastaste un 18% más en comida que el mes anterior.</h2><p className="mt-4 text-sm leading-6 text-emerald-900/70">Revisar las compras de fin de semana puede ayudarte a entender el cambio.</p></AppCard>
    </div>
