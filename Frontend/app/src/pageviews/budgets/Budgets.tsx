...
export function Budgets({ locale }: { locale: 'es' | 'en' }) {
  ...
  const periodLabel = new Intl.DateTimeFormat(locale, { month: 'long', year: 'numeric' }).format(new Date());
  return <div className="space-y-6">
    <AppCard className="grid gap-5 p-5 md:grid-cols-2 xl:grid-cols-[1.15fr_repeat(3,1fr)_1.4fr] xl:items-center">
      <div><p className="text-sm font-bold">Resumen del mes</p><span className="mt-3 flex h-11 items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-semibold"><CalendarDays size={17}/>{periodLabel}</span></div>
      ...
      <div className="xl:border-l xl:border-slate-100 xl:pl-5"><p className="text-xs font-semibold text-slate-500">Gastado total</p><strong className="mt-2 block text-2xl">{formatMoney(spent)}</strong><span className="font-semibold text-orange-600">{total?Math.round(spent/total*100):0}% del presupuesto</span></div>
      <div className="xl:border-l xl:border-slate-100 xl:pl-5"><p className="text-xs font-semibold text-slate-500">Disponible</p><strong className="mt-2 block text-2xl text-emerald-700">{formatMoney(total-spent)}</strong><span className="text-slate-500">{total?Math.round((total-spent)/total*100):0}% restante</span></div>
