...
export function Topbar({ locale, onMenuAction: onMenu, onNotificationsAction: onNotifications }: { locale: 'es' | 'en'; onMenuAction: () => void; onNotificationsAction: () => void }) {
  ...
  return <header className="z-30 flex h-16 shrink-0 items-center gap-2 bg-[#f7f9f8] px-3 dark:bg-slate-950 sm:px-5 lg:grid lg:h-24 lg:grid-cols-[minmax(16rem,1fr)_minmax(18rem,28rem)_auto] lg:gap-6 lg:px-7">
    ...
    <div className="ml-auto flex items-center gap-1.5 sm:gap-2 lg:ml-0 lg:justify-self-end">
      <Link href={locale === 'es' ? '/en/dashboard' : '/es/dashboard'} className="flex h-10 items-center gap-1.5 rounded-xl border border-slate-200 px-2 text-slate-600 hover:bg-slate-50" aria-label={locale === 'es' ? 'Idioma actual: Español. Cambiar a inglés' : 'Current language: English. Switch to Spanish'}><Globe2 size={21}/><span className="text-xs font-black">{locale.toUpperCase()}</span></Link>
      <button onClick={() => dispatch({ type: 'updatePreferences', payload: { darkMode: !preferences.darkMode } })} className="grid size-10 place-items-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50" aria-label="Cambiar tema">{preferences.darkMode ? <Sun size={22}/> : <Moon size={22}/>}</button>
