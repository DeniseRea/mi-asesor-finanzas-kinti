...
export function Sidebar({ currentLocale, open = false, onClose }: { currentLocale: 'es' | 'en'; open?: boolean; onClose?: () => void }) {
  ...
  return <aside className={`fixed inset-y-0 left-0 z-50 flex h-dvh w-[17rem] flex-col overflow-hidden bg-gradient-to-b from-[#063f31] to-[#002d25] p-4 text-white shadow-2xl transition-transform duration-300 lg:relative lg:inset-auto lg:w-[clamp(14rem,17vw,17rem)] lg:shrink-0 lg:translate-x-0 lg:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}>
    <div className="mb-7 flex items-center justify-between px-2"><Link href={`/${currentLocale}/dashboard`} className="flex items-center gap-2"><Image src="/assets/login/kinti-bird.png" alt="Kinti" width={42} height={34} className="object-contain"/><span className="text-2xl font-black tracking-tight">kinti</span></Link><button onClick={onClose} className="rounded-lg p-2 hover:bg-white/10 lg:hidden" aria-label="Cerrar menú"><X size={20}/></button></div>
    ...
    <div className="mt-3 shrink-0 rounded-2xl bg-emerald-500/12 p-4 ring-1 ring-inset ring-white/8"><div className="flex items-center gap-3"><Image src="/assets/app/whatsapp.svg" alt="WhatsApp" width={28} height={28}/><div><p className="text-xs font-bold">Conecta tu WhatsApp</p><p className="mt-1 text-[11px] leading-4 text-emerald-50/65">Registra y consulta tus movimientos desde el bot.</p></div></div></div>
