...
type Tone = 'green' | 'red' | 'violet' | 'orange';

function Sparkline({ points, stroke, tone }: { points: number[]; stroke: string; tone: Tone }) {
  ...
  return (
    <svg aria-hidden="true" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className={`metric-sparkline-${tone} h-full w-full`}>
      <defs>
        <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
          <stop className="metric-sparkline-fill-start" offset="0" stopColor={stroke} stopOpacity=".28" />
          <stop className="metric-sparkline-fill-end" offset="1" stopColor={stroke} stopOpacity=".015" />
        </linearGradient>
      </defs>
